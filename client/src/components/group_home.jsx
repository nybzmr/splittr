...
function GroupHome(props) {
  ...
  return <div className="groups-page">
    ...
    <div className="group-create-grid">
      <form className="action-card" onSubmit={createGroup}><div className="card-icon">＋</div><div><span className="eyebrow">Start fresh</span><h2>Create a group</h2><p>Set up a shared ledger and invite your friends.</p></div><input value={groupName} onChange={(e) => setGroupName(e.target.value)} placeholder="e.g. Goa trip" maxLength={50} /><button className="button button-primary" disabled={!!busyAction}>{busyAction === "create" ? "Creating…" : "Create group"}</button></form>
      <form className="action-card" onSubmit={joinGroup}><div className="card-icon accent">↗</div><div><span className="eyebrow">Have an invite?</span><h2>Join a group</h2><p>Enter the 8-character invite code shared by a group member.</p></div><input value={inviteCode} onChange={(e) => setInviteCode(e.target.value.toUpperCase())} placeholder="AB12CD34" maxLength={8} /><button className="button button-secondary" disabled={!!busyAction}>{busyAction === "join" ? "Joining…" : "Join group"}</button></form>
    </div>
