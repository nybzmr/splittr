...
function App() {
  ...
  useEffect(() => { if (token && selectedGroupId) refreshData(); }, [token, selectedGroupId, refreshData]);
  ...
  const selectGroup = (id) => { localStorage.setItem("splittr_group", id); setSelectedGroupId(id); window.history.replaceState({}, "", `?group=${encodeURIComponent(id)}`); };
  const leaveGroupView = () => { localStorage.removeItem("splittr_group"); setSelectedGroupId(""); window.history.replaceState({}, "", window.location.pathname); };
